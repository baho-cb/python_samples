...
You have to supply the frames !!!

Same as analyze_cluster_shear.py, this one takes a single gsd and records the evolution
of the cluster properties to make sure that our clusters are equilibrated so this one
is not going to be used often. (mainly for run_118, see OneNote 31.1)
...
required=False, help="the name of the output data file sth like 240x " )
...
clusters = []
sim_names = []
in_file=gsd.fl.GSDFile(name=input_files, mode='rb',application="hoomd",schema="hoomd", schema_version=[1,0])
N_frames = in_file.nframes

frames = np.arange(300,N_frames-2)
for i_f,target_frame in enumerate(frames):

    file_name = input_files[:-4]
    file = file_name.split("_")
    shape_id = file[1]
    shape_id = int(shape_id)
    cluster_utils = ClusterUtils(input_files,target_frame)
